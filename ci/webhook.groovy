/**
 * Webhook Service CI — loaded and executed by the master Jenkinsfile.
 *
 * Tech stack : Java / Maven (multi-module, uses -pl webhook -am)
 * Phases     : Phase 1 — Test (unit tests w/ 80% coverage gate, checkstyle, OWASP dependency-check)
 *              Phase 2 — Build artifacts + Docker image
 */

def call() {
    def serviceName    = 'webhook'
    ...
    try {
        ...
        stage("${serviceName}: Phase 1 - Code Quality") {
            ...
            echo "Checkstyle report generated at: ${serviceName}-checkstyle-result.xml"
        }
            ...
            try {
                try {
                    withCredentials([string(credentialsId: 'nvd-api-key', variable: 'NVD_API_KEY')]) {
                        sh '''
                            mvn -pl webhook -am org.owasp:dependency-check-maven:check \
                              -DfailBuildOnCVSS=7 \
        ...
        echo "Webhook service build completed successfully"
    } catch (Exception e) {
        echo "Webhook service build failed"
        throw e
    }
}
